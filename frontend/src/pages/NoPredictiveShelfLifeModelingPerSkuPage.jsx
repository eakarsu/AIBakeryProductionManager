...
export default function NoPredictiveShelfLifeModelingPerSkuPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api('/gap-no-predictive-shelf-life-modeling-per-sku/run', { method: 'POST', body: { input } });
      setResult(r);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No predictive shelf-life modeling per SKU</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-predictive-shelf-life-modeling-per-sku/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
