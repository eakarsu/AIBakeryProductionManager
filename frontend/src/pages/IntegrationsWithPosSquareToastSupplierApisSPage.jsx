...
// Feature: Integrations with POS (Square, Toast), supplier APIs (Sysco, US Foods), inventory (MarginEdge)
import React, { useState } from 'react';
import { api } from '../utils';

export default function IntegrationsWithPosSquareToastSupplierApisSPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api('/cf-integrations-with-pos-square-toast-supplier-apis-s/run', { method: 'POST', body: { input } });
      setResult(r);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Integrations with POS (Square, Toast), supplier APIs (Sysco, US Foods), inventory (MarginEdge)</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-integrations-with-pos-square-toast-supplier-apis-s/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
