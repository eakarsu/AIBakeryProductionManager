...
// Feature: Video-based recipe adaptation: upload finished product, AI generates scaled recipe + visual guide
import React, { useState } from 'react';
import { api } from '../utils';

export default function VideoBasedRecipeAdaptationUploadFinishedProdPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api('/cf-video-based-recipe-adaptation-upload-finished-prod/run', { method: 'POST', body: { input } });
      setResult(r);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Video-based recipe adaptation: upload finished product, AI generates scaled recipe + visual guide</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-video-based-recipe-adaptation-upload-finished-prod/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
