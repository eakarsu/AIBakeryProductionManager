...
export default function NoAgenticSupplierSubstitutionDecisioningSurfaPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api('/gap-no-agentic-supplier-substitution-decisioning-surfa/run', { method: 'POST', body: { input } });
      setResult(r);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No agentic supplier-substitution decisioning surfaced via UI</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-agentic-supplier-substitution-decisioning-surfa/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
