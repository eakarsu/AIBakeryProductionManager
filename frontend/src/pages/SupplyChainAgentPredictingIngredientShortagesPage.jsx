...
// Feature: Supply chain agent predicting ingredient shortages and auto-suggesting substitutions weeks in advance
import React, { useState } from 'react';
import { api } from '../utils';

export default function SupplyChainAgentPredictingIngredientShortagesPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api('/cf-supply-chain-agent-predicting-ingredient-shortages/run', { method: 'POST', body: { input } });
      setResult(r);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Supply chain agent predicting ingredient shortages and auto-suggesting substitutions weeks in advance</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-supply-chain-agent-predicting-ingredient-shortages/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
