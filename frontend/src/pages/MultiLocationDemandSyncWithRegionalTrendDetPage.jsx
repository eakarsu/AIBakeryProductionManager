...
// Feature: Multi-location demand sync with regional trend detection
import React, { useState } from 'react';
import { api } from '../utils';

export default function MultiLocationDemandSyncWithRegionalTrendDetPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api('/cf-multi-location-demand-sync-with-regional-trend-det/run', { method: 'POST', body: { input } });
      setResult(r);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Multi-location demand sync with regional trend detection</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-multi-location-demand-sync-with-regional-trend-det/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
