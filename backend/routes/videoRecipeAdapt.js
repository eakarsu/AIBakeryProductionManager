/*
 * Video-based recipe adaptation: Upload a finished product, AI generates scaled recipe + visual guide
 *
 * Custom Feature Suggestion (apply pass 6 — audit batch_01)
 * Suggestion: Upload a finished product, AI generates scaled recipe + visual guide
 *
...
router.post('/run', async (req, res) => {
  try {
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({ error: 'AI unavailable', missing: 'OPENROUTER_API_KEY', feature: 'video_recipe_adapt' });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${'Video-based recipe adaptation'}\n\nReturn JSON {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const systemPrompt = `You are a $'expert pastry chef adapting recipes from a video reference'. Respond with strict JSON.`;
    const aiResp = await callOpenRouter(userPrompt, systemPrompt);
    const content = aiResp?.choices?.[0]?.message?.content ?? aiResp?.raw ?? '';
    const parsed = parseAIJson(content);
    return res.json({ feature: 'video_recipe_adapt', model: aiResp?.model, parsed, raw: content });
  } catch (err) {
...
router.get('/health', (req, res) => res.json({ feature: 'video_recipe_adapt', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
