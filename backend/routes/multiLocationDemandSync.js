/*
 * Multi-location demand sync: Sync demand across multiple bakery locations with regional trend detection
 *
 * Custom Feature Suggestion (apply pass 6 — audit batch_01)
 * Suggestion: Sync demand across multiple bakery locations with regional trend detection
 *
...
router.post('/run', async (req, res) => {
  try {
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({ error: 'AI unavailable', missing: 'OPENROUTER_API_KEY', feature: 'multi_location_demand' });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${'Multi-location demand sync'}\n\nReturn JSON {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const systemPrompt = `You are a $'regional demand analyst across bakery locations'. Respond with strict JSON.`;
    const aiResp = await callOpenRouter(userPrompt, systemPrompt);
    const content = aiResp?.choices?.[0]?.message?.content ?? aiResp?.raw ?? '';
    const parsed = parseAIJson(content);
    return res.json({ feature: 'multi_location_demand', model: aiResp?.model, parsed, raw: content });
  } catch (err) {
...
router.get('/health', (req, res) => res.json({ feature: 'multi_location_demand', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
